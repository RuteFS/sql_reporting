/*
  ...
  ----------------------------------------------------
  Purpose:
  - This script was created to populate the normalized tables (patients, appointments, appointment_details)
    from the cleaned staging table (appointments_staging).
  - I replace natural keys (patient_id, appointment_id) as primary keys with synthetic
  ...
  - The base tables (patients, appointments, appointment_details) were created 
    using 01_bronze_schema.sql and 04_create_silver_tables.

  Why I use synthetic keys:
  - patient_id and appointment_id from the dataset are not guaranteed to be globally unique.
...
-- First, I find and drop the foreign key in appointments that references patient_id,
-- since I’ll replace it with patient_pk.
SELECT CONSTRAINT_NAME
FROM information_schema.KEY_COLUMN_USAGE
WHERE TABLE_SCHEMA = 'healthcare_reporting'
  AND TABLE_NAME = 'appointments'
  AND COLUMN_NAME = 'patient_id';  -- returns: appointments_ibfk_1
...
-- I add a patient_pk column to appointments so the two tables can be linked by the new synthetic key.
...
WHERE TABLE_SCHEMA = 'healthcare_reporting'
  ...
  AND COLUMN_NAME = 'appointment_id';  -- returns: appointment_details_ibfk_1
...
-- linking them to appointments via appointment_pk and
-- since appointment_id is not unique, 
-- to disambiguate which row in appointments corresponds to which row in appointments_staging
-- I join patients table using patient_id.

That’s where patient_pk comes in — not because we’re inserting it, but because it helps us uniquely identify the correct appointment_pk
.
INSERT INTO appointment_details (appointment_pk, appointment_day, sms_received, no_show)
SELECT 
    appt.appointment_pk,
    staging.appointment_day,
    staging.sms_received,
    staging.no_show
FROM 
    appointments_staging staging
JOIN 
    patients p ON 
    a.patient_id = p.patient_id AND
    a.gender = p.gender AND
...
JOIN 
    appointments appt ON 
        staging.appointment_id = appt.appointment_id AND
        staging.scheduled_day = appt.scheduled_day AND
